...
Heavily Modified version of code to make minimal sequential bam processing fast.
Originally from:
...
import bamnostic_mod
from bamnostic_mod import bgzf, bai, bam
from bamnostic_mod.utils import *
...
class AlignedSegment(object):
    ...
    def _unpack_data(self):
        ...
        #Mod Set ref start here because mod does not unpack reference attrs
    ...
    def _decode_cigar(self):
        ...
        if self._n_cigar_op != 0:
            ...
            self._cigartuples = [
                Cigar(
                    bamnostic_mod.utils._CIGAR_OPS[op[1]][1],
                    op[0],
                    op[1],
                    bamnostic_mod.utils._CIGAR_OPS[op[1]][0],
                )
            ...
            #mod - calculate sequence stop position here instead of later.
            self.__reference_end = self.__reference_start
            for op, len in self.cigartuples:
                if op in [0, 2, 3, 7, 8]:
                    self.__reference_end += len 
        else:
            self.cigar = None
